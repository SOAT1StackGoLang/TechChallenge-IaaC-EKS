...
# Search for the Load Balancer created by the K8s service
data "aws_lb" "eks_app" {
  #name = "aec7ece9930bf48a58d769d170a47238"
  tags = {
    "kubernetes.io/service-name" = "${var.project_name}/${var.lb_service_name}"
    "kubernetes.io/cluster/${var.project_name}" = "owned"
  }
}

# Get the Listener of the Load Balancer created by this Load Balancer
data "aws_lb_listener" "eks_app" {
  load_balancer_arn = "${data.aws_lb.eks_app.arn}" 
...
resource "aws_apigatewayv2_integration" "api_integration" {
  ...
  integration_uri        = data.aws_lb_listener.eks_app.arn
